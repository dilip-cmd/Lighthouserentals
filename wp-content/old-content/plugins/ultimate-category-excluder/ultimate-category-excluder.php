	...
	$cat_args = array(
 	...
 	//if( !empty($product_categories) ){
		...
		<tr<?php if ( $alt == 1 ) { echo ' class="alternate"'; $alt = 0; } else { $alt = 1; } ?>>
			<th scope="row"><?php echo $category->name; //. ' (' . $cat->cat_ID . ')'; ?></th>
			<td><input type="checkbox" name="exclude_main[]" value="<?php echo $category->term_id ?>" <?php if ( in_array( '-' . $category->term_id, $options['exclude_main'] ) ) { echo 'checked="true" '; } ?>/></td>
			
		</tr>	
	<?php } ?>
	</table>

	<p class="submit"><input class="cat_exclude" type="submit" value="<?php _e('Update', 'ultimate-category-excluder'); ?>" /></p>
...
function ksuce_exclude_categories( $query ) {
	...
	if ( $query->is_shop && !in_array_recursive( 'WPSEO_Video_Sitemap', $backtrace ) ) {
		$mbccount = 0;
