    ...
    var wpmc_checkout = {
        ...
        init: function() {
            ...
            $("#wpmc-next, #wpmc-skip-login").on("click", function() {
                var validated = self.validate_fields(self.current_index());
                console.log('Next ----')
                if (validated == true) {
        ...
        validate_fields: function(theIndex) {
            var self = this;
            var validated = true;
            console.log('validate_field theIndex - ' + theIndex)
            var wrapper = $('.wpmc-step-item.current');
            if (theIndex < 2) {
                wrapper.find('input').each(function() {
                    var validate = self.validate($(this));
                    if (validate == false) {
                        validated = false;
                    }
                });
        ...
        validate: function(field) {
            ...
            var value = field.val();
            if (name == 'billing_address_2' || name == 'billing_state') {
                return true;
            }
            if (value != '') {
