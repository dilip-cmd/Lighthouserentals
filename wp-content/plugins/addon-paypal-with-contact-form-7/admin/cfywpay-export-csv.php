...
class CF7WPAY_Expoerts_CSV {

    public function cfywpay_headers( $filename ) {
    ...
    public function cfywpay_array2csv(array &$array, $df) {
    ...
    public function cfywpay_csv_file() {
        ...
        if( isset($_REQUEST['csv']) && isset( $_REQUEST['nonce'] ) ){
            ...
            $ocpage_id         = sanitize_text_field($_REQUEST['cfywpay_formid']);
            ...
            $this->cfywpay_headers( "cfdb7-" . date("Y-m-d") . ".csv" );
            ...
            for( $p = 0; $p <= $total_query; $p++ ){
                ...
                echo $this->cfywpay_array2csv( $data, $df );
...
add_action( 'init', 'cfywpay_export_csv' );
function cfywpay_export_csv() {
    $csv = new CF7WPAY_Expoerts_CSV();
